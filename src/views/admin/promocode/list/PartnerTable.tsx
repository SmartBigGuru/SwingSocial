...
const PartnerTable = forwardRef<RefreshHandle>(({ }, ref) => {
  ...
  const changeParam = () => {
    ...
    router.push(`/admin/sp/promocode-manage/${queryString ? `?${queryString}` : ''}`)
  }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const apiUrl = `/api/admin/promocode?${queryParams.toString()}`;
  ...
  const columns = useMemo<ColumnDef<TableAction, any>[]>(
    ...
    () => [
      ...
      columnHelper.accessor('PromoCodeText', {
        header: 'Code',
        cell: ({ row }) => <Typography>{row.original.PromoCodeText}</Typography>
      }),
      columnHelper.accessor('ExpireDate', {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Typography>
              {new Date(row.original.ExpireDate).toLocaleDateString('en-US')}
            </Typography>
          );
        }
      }),
