...
const Content = forwardRef<InvoiceHandle>((props, ref) => {
  // States
  ...
  const [items, setItems] = useState<InvoiceItemType[]>([
    { contract_id: undefined, retainer: 0, unit_price: 0 }
  ])
  ...
  useImperativeHandle(ref, () => ({
    ...
    save: () => insertInvoice('Draft'),
    send: () => insertInvoice('Sent'),
    download: () => {  }
  }))

  const insertInvoice = async (status: string) => {
    let itemError = false
